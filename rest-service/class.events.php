			...
			switch (strtoupper($this->request->pathElements[2]))
			{
				...
				case "DATE" :
					...
					****************/
					...
					if (isset($this->request->pathElements[5]))
					{
						if (strToUpper($this->request->pathElements[5]) != "JSON")
						{
							$daysInMonth = cal_days_in_month(CAL_GREGORIAN,
															$this->request->pathElements[4],
															$this->request->pathElements[3]);
...
protected function isValidInt($value, $min, $max)
{
  ...
  protected function handleAllEvents()
  {
		if (! isset($this->request->pathElements[2]))
		{
				$this->throwError("INVALID year - no year or seven day URI flag specified", 15);
		}
		else
		{
			if (strToUpper($this->request->pathElements[2]) != "DAYS")
			{
	...
	if (isset($this->request->pathElements[4]) && strToUpper($this->request->pathElements[4]) != "JSON")
	{
		...
		if (! empty($this->request->pathElements[4]))
		{
			...
			{
				$this->throwError("INVALID day  [" . $this->request->pathElements[4] . "]", 160);
			}
	...
	else
	{
		...
		{
			$oCalendar = new CalendarNumberOfDays($this->request->pathElements[3]);
			$oCalendar->writeAllEventsForDays($this);
		}
	}
