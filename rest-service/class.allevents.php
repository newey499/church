...
10/03/2011  CDN   	Add support for extracting events for a single day as well as
					a calendar month
18/04/2012  CDN   	Fucking MySQL - changed the syntax.
					Creates memory table = "TYPE = HEAP" now gives error so use "ENGINE = MEMORY"
**************************************************/
...
CDN 		18/04/2012 	Create CalendarNumberOfDays Class by extending Calendar Class for producing <n>
						days worth of events starting from today.
*************************************/


Class CalendarNumberOfDays extends Calendar
{
	protected $startDate;
	protected $endDate;
	protected $numberOfDaye;

	// CDN 18/04/2012 Constructor for producing $numberOfDays days worth of events starting from today
	function __construct($numberOfDays)
	{
		//die("number of days [" . $numberOfDays . "]");
		$this->numberOfDays = $numberOfDays;
		$this->sourceTableFlags = Calendar::TABLE_ALL;
		$this->startDate = new DateTime();
		$this->endDate = new DateTime();
		$this->endDate->modify("+" . $this->numberOfDays . " day");  // pick up last seven days of previous month

		$sourceTables = Calendar::TABLE_ALL;
		//$day 	= date("j");
		//$month 	= date("n");
		//$year	= date("Y");
		$this->createTable();
	...
	protected function loadForthcomingevents()
	{
		...
		$qry .= "WHERE ( ";
		$qry .= "    		( DATE_FORMAT(eventdate, \"%Y-%m-%d\" ) >= \"" . $this->startDate->format("Y-m-d") . "\" )" ;
		$qry .= "  		AND ";
		$qry .= "    		( DATE_FORMAT(eventdate, \"%Y-%m-%d\" ) <= \"" . $this->endDate->format("Y-m-d") . "\" )" ;
		$qry .= "  		AND ";
		$qry .= "    		( isvisible = \"YES\" )";
		$qry .= "      ) ";

		//print("start [" . $this->startDate->format("Y-m-d") . "] end [" . $this->endDate->format("Y-m-d") . "]");
		//print("<h4>$qry</h4>");

		if (mysql_query($qry) == FALSE)
		{
			die("Failed to Import rows from forthcomingevents [" . mysql_error() . "]");
		}
	...
	{
		$date = new DateTime();
		// Explicitly set the date, although the default constructor sets the date/time to NOW
		$date->setDate(	$this->startDate->format("Y"),
		...
		while ( $this->processDate($date, $this->endDate)  )
		{
			for ($dow= 1; $dow <= 7; $dow++)
			{
				...
				if (! $this->processDate($date, $this->endDate))
				{
					break;
				}
...
// ====================================================================
// Write all events for next seven days
public function writeAllEventsForDays(Events $oEvents)
{
	$qry  = " select * from calendar " ;
	$qry .= " where date(eventdate) >= '" . date('Y-m-d') . "' ";
	$qry .= " order by eventdate, eventtime";
	$cursor = mysql_query($qry);
	if ($cursor == FALSE)
	{
		die(mysql_error());
	}
	...
	//$oEvents->request->writeElementStart("allevents");

	$oEvents->request->writeDateElement("startdate", $this->startDate->format("Y-m-d"));
	$oEvents->request->writeElement("numberofdays",  $this->numberOfDays);
	$oEvents->request->writeElement("recordcount",  mysql_num_rows($cursor));

	//$oEvents->request->writeElementEnd("allevents");
	...
	while ($row = mysql_fetch_assoc($cursor))
	{
		...
		$oEvents->request->writeElement("parentid",  $row['parentid']);
		...
		//$oEvents->request->writeElement("eventdate",  $row['eventdate']);
		//$oEvents->request->writeElement("eventtime",  $row['eventtime']);
		$oEvents->request->writeElement("eventname",  $row['eventname']);
		$oEvents->request->writeElement("isvisible",  $row['isvisible']);

		// $oEvents->request->writeElement("linkurl",  htmlentities($row['linkurl']));
		if ($row['eventsource'] == 'FORTHCOMING')
		{
			...
			if (! empty($row['linkurl']) )
			{
				...
				else
				{
					...
					$str = "http://" . WEBSITE_DOMAIN . "/" . $row['linkurl'];
				}
...
} // END Class CalendarNumberOfDays extends Calendar
// ====================================================================
...
protected function createTable()
{
	...
	if ($this->useMemoryTable)
	{
		$errMsg = "Failed to create temporary calendar table (1))";
		$qry  =  "CREATE TEMPORARY TABLE calendar ";
	}
	else
	{
		$errMsg = "Failed to create calendar table (2))";
	...
	$qry .= ") ENGINE = MEMORY; ";	// CDN 18/04/2012 "TYPE = HEAP" now gives error
